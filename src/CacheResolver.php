 ...
 */
class CacheResolver implements CacheInterface
{
	 ...
	 */
	public static function setCacheResolver($cache)
	{
	...
	{
		if (empty(static::$cacheSingleton)) {
			throw new InvalidArgumentException('未传入 \Psr\SimpleCache\CacheInterface 实现,请先调用 CacheResolver::setCacheResolver($cacheInterface).');
		}
		$this->cache = static::$cacheSingleton;
	}
	...
	{
		return $this->getCache()->set($key, $value, $ttl ?? static::FOREVER);
	}
	...
	{
		return $this->getCache()->setMultiple($values, $ttl ?? static::FOREVER);
	}
